...
class ElevatorController(ABC):
    ...
    def _run_event_driven_simulation(self) -> None:
        ...
        try:
            ...
            print(f"[ALGO] Max tick for this traffic round: {self.current_traffic_max_tick}")
            ...
            while self.is_running:
                # 检查是否达到最大tick数
                if self.current_tick >= self.current_traffic_max_tick:
                    print(f"[ALGO] Reached max tick ({self.current_tick} >= {self.current_traffic_max_tick})")
                    break
                
                tick_count += 1
                print(f"[ALGO] Processing tick {tick_count}, current_tick={self.current_tick}, max={self.current_traffic_max_tick}")
                ...
                # 处理事件
                if events:
                    print(f"[ALGO] Tick {self.current_tick}: Processing {len(events)} events")
                    for event in events:
                        print(f"[ALGO]   Event: {event.type.value}")
                        self._handle_single_event(event)
                else:
                    print(f"[ALGO] Tick {self.current_tick}: No events")
                ...
                if self.current_tick >= self.current_traffic_max_tick:
                    ...
                    print(f"\n[INFO] 当前场景已完成 (tick={self.current_tick}/{self.current_traffic_max_tick})")
                    print(f"[INFO] 自动切换已禁用，请手动切换或停止模拟")
                    break  # 停止，不自动切换
