...
class ElevatorSimulation:
    ...
    def _process_elevator_stops(self) -> None:
        ...
        for elevator in self.elevators:
            ...
            # 🔑 关键修复：无论上一个tick的方向如何，只要电梯停止且有待接乘客，就让他们上梯
            self._process_passenger_in(elevator)
            
            # 检查是否需要发出IDLE事件（在没有下一个目标且没有待接乘客时）
            floor = self.floors[current_floor]
            if elevator.next_target_floor is None and len(floor.up_queue) == 0 and len(floor.down_queue) == 0:
    ...
    def _set_elevator_target_floor(self, elevator: ElevatorState, floor: int) -> None:
        ...
        # 关键修复：如果目标楼层与当前楼层不同，且电梯处于STOPPED状态，立即启动电梯
        if elevator.current_floor != floor and elevator.run_status == ElevatorStatus.STOPPED:
            elevator.run_status = ElevatorStatus.START_UP
            server_debug_log(f"✓ 电梯 E{elevator.id} 从STOPPED启动到START_UP，目标方向: {elevator.target_floor_direction.value}")
            return  # 直接返回，后续的状态检查由下一个tick处理
        ...
        if elevator.current_floor != floor or elevator.position.floor_up_position != 0:
            old_status = ElevatorStatus.STOPPED.value if original_target_floor == elevator.current_floor else elevator.run_status.value
            server_debug_log(f"✓ 电梯{elevator.id} 状态转变完成 新状态:{elevator.run_status.value}")
...
def step_simulation() -> Response | tuple[Response, int]:
    try:
        ...
        server_debug_log(f"ERROR in step_simulation: {error_msg}")
        server_debug_log(f"Traceback:\n{error_trace}")
        print(f"[ERROR] /api/step failed: {error_msg}", flush=True)
        print(f"[TRACEBACK]\n{error_trace}", flush=True)
        return json_response({"error": error_msg, "traceback": error_trace}, 500)
